...
author: "Daniel Arellano Martínez, Bruno González Arenas, Carlos González Arenas, Víctor Manuel Vázquez García"
output:
...
La fase de transformación implica la modificación y limpieza de datos para que puedan ser utilizados posteriormente por el proceso DM (Data Mining).
...
class_red_train <- mutate(red_train, cluster = cut_dend)
rm(k, cut_dend, hc)
head(class_red_train)
...
ggplot(class_red_train, aes(x = distance, y = trip_duration, color = factor(cluster))) + geom_point()
...
2. **Obtener EPS**
Mediante la función kNNdist se puede determinar el EPS que se va a utilizar.
```{r}
...
3. **Aplicar función dbscan**
Se aplica la función dbscan para categorizar en los diferentes clusters.
```{r}
...
4.  **Representación de clusters**
Se representan los diferentes clústeres extraídos mediante densidad.
